{config, pkgs, ...}: {
  fonts.fontconfig = {
    enable = true;
    antialiasing = true;
    hinting = "slight";          # subtle hinting — preserves letterforms
    subpixelRendering = "rgb";   # standard LCD; change to "none" on OLED
    defaultFonts = {
      serif      = [ "Noto Serif"              "Noto Color Emoji" ];
      sansSerif  = [ "Inter"   "Noto Sans"     "Noto Color Emoji" ];
      monospace  = [ "JetBrainsMono Nerd Font" "Fira Code" "Noto Color Emoji" ];
      emoji      = [ "Noto Color Emoji" ];
    };
  };

  qt = {
    enable = true;
    platformTheme.name = "kde";
    style.name = "breeze";
  };

  gtk = {
    enable = true;
    font = {
      name = "Inter";
      size = 11;
    };
    theme = {
      name = "adw-gtk3-dark";
      package = pkgs.adw-gtk3;
    };
    iconTheme = {
      name = "Papirus";
      package = pkgs.papirus-icon-theme;
    };
    cursorTheme = {
      name = "Bibata-Modern-Classic";
      package = pkgs.bibata-cursors;
      size = 24;
    };
    gtk4.theme = config.gtk.theme;

    # Wallpaper colours, on top of the theme. home-manager writes gtk.css
    # itself (the theme import above), so matugen writes a second file next
    # to it and this pulls it in. The path is absolute on purpose: gtk.css is
    # a symlink into the store, and a relative @import would resolve there.
    #
    # The import lands after the theme's, and the last @define-color for a
    # name wins, so these override adw-gtk3-dark. If matugen has never run,
    # GTK logs a failed import and carries on with the plain theme.
    gtk3.extraCss = ''
      @import url("file://${config.home.homeDirectory}/.config/gtk-3.0/matugen.css");
    '';
    gtk4.extraCss = ''
      @import url("file://${config.home.homeDirectory}/.config/gtk-4.0/matugen.css");
    '';
  };

  # Apply the same cursor in Wayland sessions
  home.pointerCursor = {
    # Explicit since home-manager 25.05: setting the options below no longer
    # implies enable, and relying on that is deprecated.
    enable = true;
    name = "Bibata-Modern-Classic";
    package = pkgs.bibata-cursors;
    size = 24;
    gtk.enable = true;
  };

  # kdeglobals (Dolphin, Ark, Okular colors) is generated by matugen — see matugen.nix.

  home.packages = with pkgs; [
    # Qt/KDE theming
    kdePackages.breeze
    kdePackages.breeze-icons
    # UI & system fonts
    inter                           # clean modern UI font
    noto-fonts
    noto-fonts-cjk-sans
    noto-fonts-cjk-serif
    noto-fonts-color-emoji
    liberation_ttf
    # Monospace
    fira-code
    fira-code-symbols
    nerd-fonts.fira-code
    nerd-fonts.jetbrains-mono
    nerd-fonts.hack
    nerd-fonts.iosevka
    nerd-fonts.noto                 # Noto with Nerd Font glyphs (good AGS fallback)
    # Decorative / specialty
    mplus-outline-fonts.githubRelease
    dina-font
    texlivePackages.cormorantgaramond
    cinzel
  ];
}
