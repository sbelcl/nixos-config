...
in {
  ...
  xdg.configFile."matugen/config.toml".text = ''
    ...
    # One template, two outputs: GTK 3 and GTK 4 read their own directory, and
    # both files are pulled in by an @import that home-manager appends to the
    # gtk.css it already owns (see fonts.nix). matugen cannot write gtk.css
    # itself — that file is a home-manager symlink into the store.
    ...
    input_path  = "~/.config/matugen/templates/nvim-base16.lua"
    output_path = "~/.config/nvim/matugen-base16.lua"
  ...
  # Neovim's colours, as a base16 palette rather than a colorscheme.
  #
  # Writing a colorscheme means naming a few hundred highlight groups and
  # keeping up with every plugin that adds more. mini.base16 already does that
  # from sixteen colours, so this template only has to produce the sixteen —
  # see ./nvim/lua/plugins/theme.lua for the twelve lines that load it.
  #
  # base00-07 are the background-to-foreground ramp and base08-0F the accents.
  # The accent roles are the ones the Alacritty audit found readable in both
  # light and dark: primary, secondary, tertiary, error and the `*_fixed_dim`
  # family. Nothing from `*_fixed` or `on_*_container`, which are container
  # tones that only work as text on the dark half.
  #
  # `.default` here, unlike hyprlock: this sits on a background that follows
  # the session mode, so its text should too.
  xdg.configFile."matugen/templates/nvim-base16.lua".text = ''
    -- Generated by matugen. Edit the template in home/modules/matugen.nix.
    return {
      ...
      base05 = "{{colors.on_background.default.hex}}",      -- default foreground
      ...
      base09 = "{{colors.tertiary_fixed_dim.default.hex}}", -- numbers, constants
      base0A = "{{colors.secondary.default.hex}}",          -- classes, search bg
      base0B = "{{colors.tertiary.default.hex}}",           -- strings, diff added
      base0C = "{{colors.primary_fixed_dim.default.hex}}",  -- escapes, regex
      base0D = "{{colors.primary.default.hex}}",            -- functions, headings
      base0E = "{{colors.secondary_fixed_dim.default.hex}}",-- keywords
      base0F = "{{colors.surface_tint.default.hex}}",       -- deprecated
    }
  '';
  ...
  # GTK colours, imported by both GTK 3 and GTK 4 (see fonts.nix).
  #
  # Two naming families, because two toolkits: libadwaita's *_bg_color /
  # *_fg_color set, which is what Nautilus, satty and every other GTK 4 app
  # here reads, and adw-gtk3's theme_* set for GTK 3. Defining both costs
  # nothing — a toolkit ignores the names it does not know.
  #
  # These land *after* the theme import in gtk.css, and in GTK CSS the last
  # @define-color for a name wins, so they override adw-gtk3-dark rather than
  # fighting it. Running apps do not reload CSS; a restart picks it up.
  xdg.configFile."matugen/templates/gtk-colors.css".text = ''
    ...
    @define-color headerbar_backdrop_color {{colors.background.default.hex}};
    ...
    @define-color sidebar_backdrop_color {{colors.background.default.hex}};
